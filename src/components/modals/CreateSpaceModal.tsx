...
import { IconPlus, IconBuildingSkyscraper, IconColorPicker, IconSparkles } from '@tabler/icons-react';
...
export function CreateSpaceModal() {
  const { isCreateSpaceModalOpen, setIsCreateSpaceModalOpen, addIndustrySpace } = useCRM();

  const [spaceName, setSpaceName] = useState('');
  const [selectedColor, setSelectedColor] = useState('#10b981');

  if (!isCreateSpaceModalOpen) return null;

  const handleCreateSpace = (e: React.FormEvent) => {
    e.preventDefault();
    if (!spaceName.trim()) return;

    addIndustrySpace(spaceName.trim(), selectedColor);
    setSpaceName('');
  ...
  return (
    <Modal
      ...
      title="Create Industry Space"
      subtitle="Separate CRM pipeline, leads directory, and deliverables per niche"
      maxWidth="max-w-[440px]"
    >
      <form onSubmit={handleCreateSpace} className="space-y-3.5 text-[11.5px]">
        {/* Space Name Input */}
        <div className="space-y-1">
          <label className="text-[10px] font-semibold text-[var(--t-font-color-tertiary)] uppercase tracking-wider block">
            Industry / Niche Space Name <span className="text-rose-500">*</span>
          </label>
          <div className="relative">
            <input
              ...
              placeholder="e.g. Restaurants & Hospitality, Solar & Roofing..."
        ...
        {/* Info Note */}
        <div className="p-2.5 rounded-[5px] bg-[var(--t-background-secondary)] border border-[var(--t-border-color-light)] text-[11px] text-[var(--t-font-color-secondary)] leading-relaxed">
          Switching between spaces dynamically isolates your pipeline, cold outreach lists, and projects for this specific industry. You can also view all leads combined in <strong>All Spaces (Global)</strong>.
        </div>
